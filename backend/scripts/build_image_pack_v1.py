#!/usr/bin/env python3
"""Build Content Pack v1 image files from external assets.

Inputs:
- assets/content_pack_v1.json (optional)
- assets/images_v1/{concept_id}.webp (fallback)

Outputs:
- backend/static/images/word_{concept_id}.webp
"""
...
def _resolve_image_source(
    ...
    image_map = manifest.get("images", {}) if isinstance(manifest, dict) else {}
    if isinstance(image_map, dict) and cid in image_map:
        candidate = repo_root / image_map[cid]
        return candidate if candidate.exists() else None
    candidate = primary_dir / f"word_{cid}.webp"
    if candidate.exists():
        return candidate
    candidate = fallback_dir / f"{cid}.webp"
    return candidate if candidate.exists() else None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Content Pack v1 images from external assets")
    ...
    parser.add_argument("--input-dir", default="assets/images_v1")
    ...
    primary_dir = repo_root / "assets" / "images"
    output_dir = repo_root / "backend" / "static" / "images"
    output_dir.mkdir(parents=True, exist_ok=True)
    placeholder_path = output_dir / "_placeholder_1x1.webp"
    ...
    for cid in concept_ids:
        src = _resolve_image_source(cid, manifest, repo_root, primary_dir, input_dir)
        dst = output_dir / f"word_{cid}.webp"
        if src is None:
            missing.append(cid)
            continue
        if dst.exists() and not args.overwrite:
            ...
            copied += 1
        if _is_placeholder(dst, placeholder_hash):
            placeholder.append(cid)

    print("--- Image Pack v1 Build ---")
    print(f"Concept IDs: {len(concept_ids)}")
    print(f"Copied files: {copied}")
    print(f"Skipped existing: {skipped}")
    print(f"Missing image IDs: {len(missing)}")
    print(f"Placeholder image IDs: {len(placeholder)}")
    if missing:
    ...
    if args.strict and missing:
        print("ERROR: Missing images in strict mode.")
        return 2
    if args.strict and placeholder:
        print("ERROR: Placeholder images present in strict mode.")
        return 3
