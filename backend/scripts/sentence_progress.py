#!/usr/bin/env python3
"""Show sentence-generation progress from shard/result files."""
...
def _read_progress(root: Path) -> tuple[int, int, int, int, int]:
    shards = sorted((root / "docs" / "sentence_shards_200").glob("sentence_shard_*.json"))
    results = sorted((root / "docs" / "sentence_results").glob("*.result.json"))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Show sentence shard/result progress")
